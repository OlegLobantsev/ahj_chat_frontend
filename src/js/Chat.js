...
export default class Chat {
  ...
  renderConnectionUser(user) {
    ...
    if (user === this.activeUser) {
      ...
      window.onbeforeunload = async () => {
        ...
        console.log(query);
        fetch(`https://ahj-chat-backend.onrender.com/${query}`, {
          method: 'DELETE',
          headers: {
  ...
  async renderChat() {
    ...
    const request = fetch('https://ahj-chat-backend.onrender.com/index/', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  renderMessage(autor, text) {
    ...
    if (autor === this.activeUser) {
      ...
      information.textContent = `You, ${dateCreate}`;
  ...
  gettingUsers() {
    const eventSource = new EventSource('https://ahj-chat-backend.onrender.com/sse');
  ...
  messaging() {
    const ws = new WebSocket('wss://ahj-chat-backend.onrender.com/ws');
